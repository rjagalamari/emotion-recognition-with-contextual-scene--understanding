...
class FusionDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        return (
            ...
            torch.as_tensor(scene, dtype=torch.float32),
...
def evaluate_loader(model, loader, classes, dev, return_predictions: bool = False) -> dict:
    ...
    with torch.no_grad():
        for face, scene, yb in loader:
            face, scene, yb = face.to(dev), scene.to(dev), yb.to(dev)
...
def main() -> None:
    ...
    output_dir = PROJECT_ROOT / "experiments" / "experiment3_3_places365_cross_gated"
    ...
    manifest.to_csv(output_dir / "manifest_experiment3_3_places365.csv", index=False)
    ...
    model = CrossGatedTokenFusionClassifier(
        face_dim=int(sample_face.shape[0]),
        context_dim=int(sample_scene.shape[0]),
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        train_bar = tqdm(loaders["train"], desc=f"experiment3_3 epoch {epoch}/{CONFIG['epochs']}", leave=False)
        for face, scene, yb in train_bar:
            face, scene, yb = face.to(dev), scene.to(dev), yb.to(dev)
            optimizer.zero_grad()
            logits = model(face, scene)
    ...
    final_metrics = {
        "branch": "vision_fusion_cross_gated",
        "input_dim": input_dim,
