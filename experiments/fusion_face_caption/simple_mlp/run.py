...
class FeatureDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        features = np.concatenate([face, semantic], axis=0)
...
def main() -> None:
    ...
    output_dir = PROJECT_ROOT / "experiments" / "experiment4_3"
    ...
    manifest.to_csv(output_dir / "manifest_experiment4_3.csv", index=False)
    ...
    sample_semantic = np.asarray(np.load(manifest.iloc[0].semantic_path), dtype=np.float32)
    input_dim = int(sample_face.shape[0] + sample_semantic.shape[0])
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        train_bar = tqdm(loaders["train"], desc=f"experiment4_3 epoch {epoch}/{CONFIG['epochs']}", leave=False)
