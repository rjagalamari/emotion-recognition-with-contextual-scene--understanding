...
def build_fusion_manifest(
    ...
    for face_path in sorted(face_root.rglob("*.npy")):
        ...
        semantic_path = semantic_root / rel
        if not semantic_path.exists():
            raise FileNotFoundError(f"Missing matching semantic embedding for {rel}: {semantic_path}")
    ...
    if manifest.empty:
        raise FileNotFoundError("No matched face/semantic embedding pairs found.")
    ...
    if train_df.empty or test_df.empty:
        raise ValueError("Expected non-empty train and test splits in the embedding folders.")
...
class FeatureDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        semantic = np.asarray(np.load(row.semantic_path), dtype=np.float32)
        label = self.label_to_id[row.label]
        return (
            torch.as_tensor(face, dtype=torch.float32),
            torch.as_tensor(semantic, dtype=torch.float32),
...
class CrossGatedTokenFusionClassifier(nn.Module):
    def __init__(
        ...
        self.face_to_context = nn.MultiheadAttention(
            embed_dim=token_dim,
            num_heads=num_heads,
            dropout=dropout,
            batch_first=True,
        )
        self.context_to_face = nn.MultiheadAttention(
            embed_dim=token_dim,
            num_heads=num_heads,
            dropout=dropout,
            batch_first=True,
        ...
        gate = self.gate(
            torch.cat(
                [
                    face_pool,
                    context_pool,
                    face_pool * context_pool,
                    torch.abs(face_pool - context_pool),
                ],
                dim=1,
            )
        )
...
def evaluate_loader(model, loader, classes, dev, return_predictions: bool = False) -> dict:
    ...
    with torch.no_grad():
        for face, semantic, yb in loader:
            face, semantic, yb = face.to(dev), semantic.to(dev), yb.to(dev)
            logits = model(face, semantic)
...
def main() -> None:
    ...
    semantic_root = PROJECT_ROOT / "experiment4_data" / "scene_text_minilm"
    output_dir = PROJECT_ROOT / "experiments" / "experiment4_4_cross_gated"
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"Using device: {dev}")
    print(f"Face embeddings: {face_root}")
    print(f"Semantic embeddings: {semantic_root}")
    ...
    manifest.to_csv(output_dir / "manifest_experiment4_4.csv", index=False)
    ...
    model = CrossGatedTokenFusionClassifier(
        face_dim=512,
        context_dim=384,
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        train_bar = tqdm(loaders["train"], desc=f"experiment4_4 epoch {epoch}/{CONFIG['epochs']}", leave=False)
        for face, semantic, yb in train_bar:
            face, semantic, yb = face.to(dev), semantic.to(dev), yb.to(dev)
            optimizer.zero_grad()
            logits = model(face, semantic)
